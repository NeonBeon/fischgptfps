...
-- Function to remove the waterfall
local function removeWaterfall()
    local waterfall = game:GetService("Workspace").world.map["Forsaken Shores"].Waterfall
    if waterfall then
        waterfall:Destroy()
        print("Waterfall removed.")
    else
...
-- Initial setup (remove players, set sky, modify environment, remove stars/clouds)
removeOtherPlayers()
setGreySky()
...
-- Remove the waterfall at the start
removeWaterfall()
